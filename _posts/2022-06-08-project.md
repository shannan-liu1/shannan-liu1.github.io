...
permalink: posts/marketflow
author: Shannan Liu
---

## Overview of Project
In this project, we designed and trained a Neural Network that can be used to make predictions on whether a specific stock's closing will go up or down next week.

We achieved this by training a neural network  on weekly observations of preprocessed financial data. The model was then deployed on a Flask web app so that users could interact with it and use it to make predictions.

You can find the published web application here: https://market-flow.herokuapp.com

### Flow of Website

You can test and use our model on the website https://market-flow.herokuapp.com by going to the "Test Our Product" tab and entering any valid yahoo-finance stock ticker into the input-box.

Here is what that would look like: 

At first, there is no user input yet:  
![_config.yml]({{ site.baseurl }}/images/test-product.png)

If the user wants to invest in Amazon, they would input the "AMZN" ticker, which would yield them the following recommendation and graphs:  

![_config.yml]({{ site.baseurl }}/images/amzn-returns-1.png)

![_config.yml]({{ site.baseurl }}/images/amzn-returns-2.png)


Here is the link to the github repo with our code for this project: https://github.com/acgowda/market-flow

## Technical Details
Now we move on to discussing the technical details of the project.
