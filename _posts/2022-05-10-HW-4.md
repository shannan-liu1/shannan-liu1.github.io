...
- Model1 (article title only) has a test accuracy of 97.66%
- Model2 (article text only) has a test accuracy of 98.26%
- Model3 (article title and text) has a test accuracy of 99.47%

## 5. Embedding Visualization
Let's visualise the embeddings that our models learned to identify patterns or associations in the words that the model found useful when distinguishing real news from fake news. We will examineboth 2-dimensional and 3-dimensional embeddings, and we will discuss a few words whose location is interpretable within each embedding.
