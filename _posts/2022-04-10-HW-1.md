...
### 5. Average Temperature of all Stations in a specific month between certain years
Finally, we'll answer this question to wrap up our report
> What is the average temperature in all regions in a specific month between certain years?

```python
def query_climate_database3(month,year_beg,year_end):
    """
    Query the temps.db database for information on the 'temperatures'
    and 'stations' tables.

    Returns
    - df: pandas dataframe with average temperature
      data on various stations in a specific month
    ...
    WHERE (T.month = {month}) AND (T.year BETWEEN {year_beg} AND {year_end})
    GROUP BY S.name
    """
...
def avg_heat_plot(month, year_beg, year_end):
    """
    ...
    """
    df = query_climate_database3(month = month,
                                 year_beg = year_beg,
                                year_end = year_end)

    # create figure
    fig = px.scatter_mapbox(df, # dataframe containing points you want to plot
                            lat = "LATITUDE", # column name for latitude information
                            lon = "LONGITUDE", # column name for longitude information
                            hover_name = "NAME", # what is the bold text that appears when you hover over
                            zoom = 0, # how much you want to zoom in - with 18, we're more zoomed in
                            height = 800, # controls height aspect ratio of the map
                            mapbox_style="carto-positron", # style of map
                            opacity = 0.5, # opacity for each data point
                            color = "mean_temp")
    ...
    fig.update_layout(title_text=f"Average Temperature in {month_d[month]}\
\nBetween {year_beg} - {year_end}")
...
Output
```python
fig = avg_heat_plot(1,1970,1980)
fig
```

{% include avg-heat-hw1.html %}
